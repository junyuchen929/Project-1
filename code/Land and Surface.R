...
extract_landcover_summary <- function(nlcd_path, year, states = c("DC", "MD", "VA")) {
  ...
  counties <- counties(state = states, year = 2020, cb = TRUE)
  counties <- st_transform(counties, crs = crs(nlcd_raster))
  
  # Step 3: Extract NAMELSAD info before sf loses it
  ...
  # Step 6: Add NAMELSAD info (this is already included, technically optional now)
  landcover_summary <- landcover_summary %>%
    left_join(county_info, by = c("STATEFP", "NAMELSAD"))
  ...
  landcover_summary <- landcover_summary %>%
    ...
    group_by(STATEFP, NAMELSAD, category) %>%
...
prepare_landcover_wide <- function(df) {
  ...
  df_wide <- df %>%
    ...
    dplyr::select(state, NAMELSAD, year, builtup, forest, agriculture, other)
...
extract_impervious_summary <- function(imperv_path, year, states = c("DC", "MD", "VA")) {
  ...
  counties <- counties(state = states, cb = TRUE, year = 2020)
  counties <- st_transform(counties, crs = crs(imperv_raster))
  
  # Step 3: Prepare full county name with LSAD (e.g. "Baltimore city")
  counties <- counties %>%
    mutate(NAME_LSAD = NAMELSAD)  # Keep full name
  
  county_info <- counties %>%
    sf::st_drop_geometry() %>%
    dplyr::select(STATEFP, NAME_LSAD)
  
  # Step 4: Compute impervious % for each county
  process_one_county <- function(i) {
    county_name <- counties$NAME_LSAD[i]  # full name e.g. "Baltimore city"
    STATEFP <- counties$STATEFP[i]
    county_geom <- counties[i, ]
  ...
  # Step 5: Join to get LSAD info if needed later
  imperv_summary <- imperv_summary %>%
    left_join(county_info, by = c("STATEFP", "NAME_LSAD"))
...
prepare_impervious_summary_by_year <- function(df, year) {
  df_prepared <- df %>%
    ...
    rename(county = NAME_LSAD) %>%
    dplyr::select(state, county, year, impervious)
